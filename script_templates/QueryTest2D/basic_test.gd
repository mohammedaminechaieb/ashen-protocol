# meta-description: Base template for QueryTest2D with basic structure and a random number test
@tool
extends QueryTest2D

# You can add as many contexts as you need for the test
## Context the test will be performed on
@export var context: QueryContext2D
...
func _perform_test(query_instance: QueryInstance2D) -> void:
	if not context:
		# query_instance stores querier, so it can default into it
		context = query_instance.querier_context
	
	# You can get the positions of the contexts as a PackedVector2Array
	var context_positions: PackedVector2Array = context.get_context_positions(query_instance)
	# You can also get the nodes within that context, as some tests could use that information
	# var context_nodes: Array[Node2D] = context.get_context_nodes(query_instance)
	...
	while query_instance.has_items():
		...
		var item: QueryItem2D = query_instance.get_next_item()
		if item.is_filtered:
			continue
		...
		for pos: Vector2 in context_positions:
